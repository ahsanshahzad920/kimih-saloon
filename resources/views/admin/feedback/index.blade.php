            ...
            <div class="container-fluid">
                <div class="card">
                    <div class="card-header">
                        <h4 class="card-title mb-0">All Feedback</h4>
                    </div><!-- end card header -->
                        ...
                        <div class="listjs-table" id="customerList">
                            <div class="row g-4 mb-3">
                                ...
                                <div class="col-sm-auto justify-content-sm-end">
                                    <div>
                                        <a href="#" data-bs-target="#exampleModalToggle"
                                        data-bs-toggle="modal" class="btn btn btn-success add-btn rounded-3 mt-2" >Add Feedback <i class="bi bi-plus-lg"></i>
                                        </a>
                                ...
                                <table class="table align-middle table-nowrap" id="example">
                                    <thead class="table-light">
                                        <tr>
                                            ...
                                            <th class="sort">Store Name</th>
                                            <th class="sort">Feedback</th>
                                            <th class="sort">Rating</th>
                                        ...
                                        @foreach ($feedbacks as $feedback)
                                            <tr>
                                                ...
                                                <td class="align-middle">
                                                    <img src="{{asset($feedback->image)}}" alt="No Image" style="width: 60px">
                                                </td>
                                                <td class="productimgname">
                                                    {{ $feedback->name }}
                                                </td>
                                                <td class="productimgname">
                                                    {{ $feedback->store->businessUser->business_name ?? ''}}
                                                </td>
                                                <td>{{ $feedback->feedback }}</td>
                                                <td>{{ $feedback->rating }}</td>
                                                <td>
                                                    <div class="form-check form-switch">
                                                        <input class="form-check-input changeStatus" {{$feedback->status == 1 ?  'checked' : ''}} type="checkbox" role="switch" id="flexSwitchCheckDefault" data-id="{{$feedback->id}}">
                                                ...
                                                <td>
                                                    <div class="d-flex gap-2">
                                                        <div class="edit">
                                                            ...
                                                            data-bs-target="#editCategoryModel{{ $feedback->id }}"
            ...
            <div class="modal-body">
                <form action="{{ route('feedback.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="form-group mt-2">
                        ...
                        <select name="store_id" id="store_id" class="form-control">
                            ...
                            @foreach ($users as $user)
                                <option value="{{$user->id}}">{{$user->businessUser->business_name}}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="form-group mt-2">
                        ...
                        <input type="number" min="0" max="6" class="form-control subheading" name="rating"
                            id="exampleFormControlInput1" placeholder="Feedback" required>
                    </div>

                    <button class="btn btn btn-success add-btn mt-4">Create</button>
                </form>
            </div>
        </div>
    </div>
...
<!-- Edit Modal STart -->
@foreach ($feedbacks as $feedback)
    <div class="modal fade" id="editCategoryModel{{ $feedback->id }}" aria-hidden="true"
                ...
                <div class="modal-body">
                    <form action="{{ route('feedback.update',$feedback->id) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            <label for="exampleFormControlInput1">Name</label>
                            <input type="text" class="form-control subheading" name="name"
                                id="exampleFormControlInput1" placeholder="Name" required value="{{$feedback->name ?? ''}}">
                        </div>
                        <div class="form-group mt-2">
                            <label for="exampleFormControlInput1">Select Store</label>
                            <select name="store_id" id="store_id" class="form-control">
                                <option disabled>Select Store</option>
                                @foreach ($users as $user)
                                    <option value="{{$user->id}}" {{$feedback->store_id == $user->id ? 'selected': ''}}>{{$user->businessUser->business_name}}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            <label for="exampleFormControlInput1">Feedback</label>
                            <input type="text" class="form-control subheading" name="feedback"
                                id="exampleFormControlInput1" placeholder="Feedback" required value="{{$feedback->feedback ?? ''}}">
                        </div>
                        <div class="form-group">
                            <label for="exampleFormControlInput1">Rating</label>
                            <input type="number" min="0" max="6" class="form-control subheading" name="rating"
                                id="exampleFormControlInput1" placeholder="Feedback" required value="{{$feedback->rating ?? ''}}">
                        </div>
    ...
    <script>
        ...
        $('.changeStatus').change(function() {
                ...
                $.ajax({
                    type: 'POST',
                    dataType: 'JSON',
                    url: '/cms/feedback-change-status',
                    data: {
                        id: id,
                        status: status,
                        _token: '{{ csrf_token() }}'
