                    ...
                    <div class="card-body">
                        <form action="{{ route('blogs.store') }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="form-group mb-4">
                                ...
                                <textarea name="tags" id="tags" class="form-control" rows="6" placeholder="Enter Tags by comma separate">{{ old('tags') }}</textarea>
                            </div>

                            <div class="form-group mb-4">
                                <label for="body">Body</label>
                                <textarea class="form-control {{ $errors->has('body') ? 'is-invalid' : '' }}" id="editor" name="body"
                                    rows="5" placeholder="Blog Content"></textarea>
                                @error('body')
                                    <span class="text-danger">{{ $message }}</span>
                                @enderror
                            </div>
