            ...
            <div class="container-fluid">
                <div class="card">
                    <div class="card-header">
                        <h4 class="card-title mb-0">Update Feature</h4>
                    </div><!-- end card header -->
                    ...
                    <div class="card-body">
                        <form action="{{ route('features.update', $feature?->id) }}" method="POST"
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col-lg-2">
                                    <img src="{{ asset(Storage::url($feature->icon ?? '')) }}" alt="icon" width="50"
                                        height="" />
                                </div>
                            </div>
                            <div class="form-group mb-4">
                                ...
                                <textarea class="form-control {{ $errors->has('description') ? 'is-invalid' : '' }}" id="description" name="description"
                                    rows="5" placeholder="Feature Description">{{ old('description', $feature->description ?? '') }}</textarea>
                                @error('description')
                                    <span class="text-danger">{{ $message }}</span>
                                @enderror
                            </div>
