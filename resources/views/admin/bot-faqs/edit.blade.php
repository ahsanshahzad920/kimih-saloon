            ...
            <div class="container-fluid">
                <div class="card">
                    <div class="card-header">
                        <h4 class="card-title mb-0">Update Bot Faqs</h4>
                    </div><!-- end card header -->

                    <div class="card-body">
                        <form action="{{ route('bot-faqs.update', $faq?->id) }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                            <div class="row mb-3">
                                <div class="col-lg-12">
                            ...
                            <div class="form-group mb-4">
                                ...
                                <textarea class="form-control {{ $errors->has('answer') ? 'is-invalid' : '' }}" id="answer" name="answer"
                                    rows="5" placeholder="Feature answer">{{ old('answer', $faq->answer ?? '') }}</textarea>
                                @error('answer')
                                    <span class="text-danger">{{ $message }}</span>
                                @enderror
                            </div>
