                        ...
                        <div class="listjs-table" id="customerList">
                            <div class="row g-4 mb-3">
                                ...
                                <div class="col-sm-auto justify-content-sm-end">
                                    <div>
                                        <a href="{{route('brands.index')}}" class="btn btn btn-success add-btn rounded-3 mt-2" >Add Brand <i class="bi bi-plus-lg"></i>
                                        </a>
                                        <a href="{{ route('categories.index') }}" class="btn btn btn-success add-btn rounded-3 mt-2" >Add Category <i
                                                class="bi bi-plus-lg"></i>
                                        </a>
                                        <a href="#" class="btn btn btn-success add-btn rounded-3 mt-2" data-bs-target="#exampleModalToggle"
                                        data-bs-toggle="modal">Add Supplier <i
                                                class="bi bi-plus-lg"></i>
                                        </a>
                                        <a href="{{route('products.create')}}" class="btn btn btn-success add-btn rounded-3 mt-2" >Add Product <i class="bi bi-plus-lg"></i>
                                        </a>
                                    </div>
                                </div>
                                ...
                                <table class="table align-middle table-nowrap" id="example">
                                    <thead class="table-light">
                                        <tr>
                                            ...
                                            <th class="sort">Barcode</th>
                                            <th class="sort">Category</th>
                                            <th class="sort">Supplier</th>
                                            <th class="sort">Quantity</th>
                                            <th class="sort">Retail Price</th>
                                        ...
                                        @foreach ($products as $product)
                                            <tr>
                                                ...
                                                <td class="align-middle">
                                                    ...
                                                    @else
                                                        <img src="{{ asset('assets/image/no-image.png') }}" alt=""
                                                ...
                                                <td class="align-middle">{{ $product->barcodes[0]['code'] ?? '' }}</td>
                                                <td class="align-middle">{{ $product->category->name ?? 'N/A'}}</td>
                                                <td class="align-middle">{{ $product->supplier->name ?? 'N/A'}}</td>
                                                <td class="align-middle">{{ $product->current_stock_quantity ?? '' }}</td>
                                                <td class="align-middle">{{ $product->retail_price ?? 'N/A' }}</td>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{ route('product.supplier.store') }}" method="POST">
                    @csrf
                    ...
                    <div class="form-group mt-2">
                        <label for="exampleFormControlTextarea1">Supplier Description</label>
                        <textarea class="form-control subheading" id="exampleFormControlTextarea1" name="description" rows="3"
                            required></textarea>
                    </div>

                    <button class="btn btn btn-success add-btn mt-4">Done</button>
                </form>
            </div>
        </div>
    </div>
        ...
        $(document).on('click', '.delSubBtn', function(e) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: `/catalogues/products/${dataId}`,
                        type: 'DELETE',
                        headers: {
