    ...
    <div class="content">
        ...
        <div class="container-fluid pt-4 px-4 mb-5">
            <div class="border-bottom">
                <h3 class="all-adjustment text-center pb-2 mb-0">Stock Orders</h3>
            </div>
                    ...
                    <div class="row my-3">
                        <div class="col-md-3 col-12 mt-2">
                            <div class="input-search position-relative">
                                <input type="text" placeholder="Search Return" class="form-control rounded-3 subheading"
                                    id="custom-filter" />
                        ...
                        <div class="col-md-9 col-12 text-end">
                            ...
                            <a href="{{ route('stock-orders.create') }}" class="btn create-btn rounded-3 mt-2">Create
                    ...
                    <table class="table" id="example">
                        <thead>
                            <tr>
                                ...
                                <th class="text-secondary">Expected Date</th>
                                <th class="text-secondary">Deliverd from</th>
                                <th class="text-secondary">Total cost</th>
                        ...
                        <tbody>
                            @foreach ($stock_orders as $order)
                                <tr>
                                    <td class="pt-3">
                                        <label for="select-checkbox" class="checkbox">
                                            <input class="checkbox__input select-checkbox deleteRow" type="checkbox"
                                                id="select-checkbox" data-id="{{ $order->id }}" />
                                    ...
                                    <td class="align-middle">{{ $order->id ?? 'N/A' }}</td>
                                    <td class="align-middle">{{ $order->expected_date ?? 'N/A' }}</td>
                                    <td class="align-middle">{{ $order->supplier->name ?? 'N/A' }}</td>
                                    <td class="text-primary align-middle">{{ $order->grand_total ?? 'N/A' }}</td>
                                    <td class="align-middle">
                                        <span class="badges green-border text-center">{{ $order->status ?? 'N/A'}}</span>
                                    </td>
                                        ...
                                        <div>
                                            ...
                                            <div class="dropdown-menu p-2 ps-0" aria-labelledby="dropdownMenuLink">
                                                <a class="dropdown-item"
                                                    href="{{ route('stock-orders.show', $order->id) }}">
                                                    <img src="{{ asset('assets/dasheets/img/menu.svg') }}"
                                                        class="img-fluid me-1" alt="" />
                                                    Detail Order
                                                </a>
                                                <a class="dropdown-item"
                                                    href="{{ route('stock-orders.edit', $order->id) }}">
                                                    <img src="{{ asset('assets/dasheets/img/menu.svg') }}"
                                                        class="img-fluid me-1" alt="" />
                                                    Edit Order
                                                </a>
                                                ...
                                                <form id="deleteForm"
                                                    action="{{ route('stock-orders.destroy', $order->id) }}"
                                                    method="POST">
                                                    @csrf
                                                    @method('DELETE')
